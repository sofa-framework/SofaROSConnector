...
namespace Zyklio
{
    namespace VelocityApproximation
    {
        ...
        SOFA_DECL_CLASS(ZyVelocityApproximator);

        int ZyVelocityApproximatorClass = core::RegisterObject("ZyVelocityApproximator.")
        .add< ZyVelocityApproximator >()
        ;

        ZyVelocityApproximator::ZyVelocityApproximator()
            : objectState(NULL)
        ...
        void ZyVelocityApproximator::bwdInit()
        {                
            ...
            if (calledByTornado)
            {
                ...
                std::cout << "(ZyVelocityApproximator::bwdInit) Getting parameters from Tornado." << std::endl;

                if (vertexEnv_x)
                {
                    vertexTornadoVec_x = generateVertexVector(vertexEnv_x);
                    std::cout << "(ZyVelocityApproximator::bwdInit) vertex_x: ";
                ...
                {
                    std::cout << "(ZyVelocityApproximator::bwdInit) ERROR: There was a remote call, but no value for the vertex x-coordinate was given. Can't run velocity approximation." << std::endl;
                ...
                if (vertexEnv_y)
                {
                    ...
                    std::cout << "(ZyVelocityApproximator::bwdInit) vertex_y: ";
                ...
                {
                    std::cout << "(ZyVelocityApproximator::bwdInit) ERROR: There was a remote call, but no value for the vertex y-coordinate was given. Can't run velocity approximation." << std::endl;
                    runVelocityApproximation = false;
                }
                ...
                if (vertexEnv_z)
                {
                    ...
                    std::cout << "(ZyVelocityApproximator::bwdInit) vertex_z: ";
                ...
                {
                    std::cout << "(ZyVelocityApproximator::bwdInit) ERROR: There was a remote call, but no value for the vertex z-coordinate was given. Can't run velocity approximation." << std::endl;
                    runVelocityApproximation = false;
                }
                ...
                if (objectEnv)
                {
                    ...
                    std::cout << "(ZyVelocityApproximator::bwdInit) object: ";
                ...
                {
                    std::cout << "(ZyVelocityApproximator::bwdInit) ERROR: There was a remote call, but no object name was given. Can't run velocity approximation." << std::endl;
                    runVelocityApproximation = false;
                }
                ...
                if (indexEnv)
                {
                    ...
                    {
                        std::cout << "(ZyVelocityApproximator::bwdInit) ERROR: Could not convert input " << indexEnv << " to an integer. Can't run velocity approximation." << std::endl;
                        runVelocityApproximation = false;
                    }
                    catch (std::out_of_range)
                    {
                        std::cout << "(ZyVelocityApproximator::bwdInit) ERROR: Input " << indexEnv << " too large. Can't run velocity approximation." << std::endl;
                        runVelocityApproximation = false;
                    }
                    std::cout << "(ZyVelocityApproximator::bwdInit) index: " << indexTornado << std::endl;
                }
                else
                {
                    std::cout << "(ZyVelocityApproximator::bwdInit) ERROR: There was a remote call, but no point index was given. Can't run velocity approximation." << std::endl;
                    runVelocityApproximation = false;
                }
                ...
                if (testRunIndexEnv)
                {
                    ...
                    {
                        std::cout << "(ZyVelocityApproximator::bwdInit) ERROR: Could not convert input " << testRunIndexEnv << " to an integer. Can't run velocity approximation." << std::endl;
                        runVelocityApproximation = false;
                    }
                    catch (std::out_of_range)
                    {
                        std::cout << "(ZyVelocityApproximator::bwdInit) ERROR: Input " << testRunIndexEnv << " too large. Can't run velocity approximation." << std::endl;
                        runVelocityApproximation = false;
                    }
                    std::cout << "(ZyVelocityApproximator::bwdInit) testRunIndex: " << testRunIndexTornado << std::endl;
                }
                else
                {
                    std::cout << "(ZyVelocityApproximator::bwdInit) ERROR: There was a remote call, but no testrun index was given. Can't run velocity approximation." << std::endl;
                    runVelocityApproximation = false;
                }
                ...
                {
                    std::cout << "(ZyVelocityApproximator::bwdInit) ERROR: The vectors for the vertex coordinates and the object names had not all the same size (";
            ...
            std::cout << "(ZyVelocityApproximator::bwdInit) objectNodePath: " << objectNodePath.getValue() << std::endl;
            std::cout << "(ZyVelocityApproximator::bwdInit) (if it crashes now, this is probably wrong)" << std::endl;
            {
            ...
            if (calledByTornado && runVelocityApproximation)
            {
                ...
                {
                    std::cout << "(ZyVelocityApproximator::bwdInit) Trying to find the mechanical state that corresponds to the object name " << tornadoObjectNameVec.at(gna) << std::endl;
                    ...
                    if (objNode)
                    {
                        ...
                        {
                            std::cout << "(ZyVelocityApproximator::bwdInit) ERROR: No MechanicalState<Vec3Types> instances found! Can't run velocity approximation." << std::endl;
                            runVelocityApproximation = false;
                        }
                        else
                        {
                            std::cout << "(ZyVelocityApproximator::bwdInit) MechanicalState<Vec3Types> instances found: " << mechanicalStates.size() << std::endl;
                            ...
                            if (collModels)
                            {
                                ...
                                {
                                    std::cout << "(ZyVelocityApproximator::bwdInit) ERROR: Could not find the a Vec3 mechanical state! Can't run velocity approximation." << std::endl;
                                    runVelocityApproximation = false;
                                }
                            }
                            else
                            {
                                std::cout << "(ZyVelocityApproximator::bwdInit) ERROR: Could not find the right mechanical state! Can't run velocity approximation." << std::endl;
                                runVelocityApproximation = false;
                            }
                        }
                    }
                    ...
                    {
                        std::cout << "(ZyVelocityApproximator::bwdInit) ERROR: Could not find a mechanical state that corresponds to " << tornadoObjectNameVec.at(gna) << ". Can't run velocity approximation." << std::endl;
                        runVelocityApproximation = false;
                    }
                }
            }
            else
            {
                ...
                std::cout << "(ZyVelocityApproximator::bwdInit) collisionNodePath: " << collisionNodePath.getValue() << std::endl;
                std::cout << "(ZyVelocityApproximator::bwdInit) (if it crashes now, this is probably wrong)" << std::endl;
            ...
            if (objectState)
            {
                std::cout << "(ZyVelocityApproximator::bwdInit) Found mechanical object: " << objectState->getName() << std::endl;
            }
            else
            {
                std::cout << "(ZyVelocityApproximator::bwdInit) Mechanical object with path \"" << objectNodePath.getValue() << "\" not found. (Probably not alarming)" << std::endl;
            }
            ...
            for (unsigned int mah = 0; mah < collisionStateVec.size(); mah++)
            {
                if (collisionStateVec.at(mah))
                {
                    if (calledByTornado)
                    {
                        std::cout << "(ZyVelocityApproximator::bwdInit) Found mechanical collision object corresponding to the name " << tornadoObjectNameVec.at(mah) << ": " << collisionStateVec.at(mah)->getName() << std::endl;
                    ...
                    {
                        std::cout << "(ZyVelocityApproximator::bwdInit) Found mechanical collision object corresponding to the path " << collisionNodePath.getValue() << ": " << collisionStateVec.at(mah)->getName() << std::endl;
                        std::stringstream tmpStr;
                    ...
                    if (runVelocityApproximation)
                    {
                        ...
                        if (calledByTornado) // if called by Tornado replace the selected vertex list from the scn file with the one given by the Tornado call
                        {
                            ...
                            std::cout << "(ZyVelocityApproximator::bwdInit) Due to the given position (" << selectedVertex << "), the following vertex on the sofa-mesh was selected: " << tmpStr.str() << std::endl;
...

                        {
                            ...
                            {
                                serr << "(ZyVelocityApproximator::bwdInit) WARNING: Selected vertex " << currentSelectedVertex << " is bigger than the number of vertices in the mesh (" << collisionStateVec.at(mah)->x.getValue().size() << "). Setting this selected vertex to 0." << std::endl;
...
//#endif
                ...
                else
                {
                    if (calledByTornado)
                    {
                        std::cout << "(ZyVelocityApproximator::bwdInit) Mechanical object with name \"" << tornadoObjectNameVec.at(mah) << "\" not found." << std::endl;
                    }
                    else
                    {
                        std::cout << "(ZyVelocityApproximator::bwdInit) Mechanical object with path \"" << collisionNodePath.getValue() << "\" not found." << std::endl;
                    }
                }
            }
            ...
            std::cout << "(ZyVelocityApproximator::bwdInit) ArbitraryController instances found: " << arbitraryControllers.size() << std::endl;

            if (arbitraryControllers.size() == 0) {
                serr << "(ZyVelocityApproximator::bwdInit) No Arbitrary Contoller found!" << std::endl;
                runVelocityApproximation = false;
            }
            else
            {
            ...
            if (runVelocityApproximation)
            {
                std::cout << "(ZyVelocityApproximator::bwdInit) All paramaters for velocity approximation are ok." << std::endl;
            }
            else
            {
                std::cout << "(ZyVelocityApproximator::bwdInit) Not all paramaters for velocity approximation are ok. Can't run velocity approximation." << std::endl;
            }

            rosTimeSteps.clear();
        }

        void ZyVelocityApproximator::reinit()
        {
        ...
        //void ZyVelocityApproximator::moveObjectToPosition(simulation::common::MechanicalOperations& mop, sofa::core::behavior::MultiVecCoord& pos/*, sofa::core::behavior::MultiVecCoord& freePos*/)
        //{
        ...
        void ZyVelocityApproximator::approximateVelocity(simulation::common::MechanicalOperations& mop, sofa::core::behavior::MultiVecCoord& pos)
        {            
            ...
            {
                {
                    ...
                    if (rosControl.getValue())
                    {
                        ...
                        if (!seqFound)
                        {
                            std::cout << "WARNING (ZyVelocityApproximator::approximateVelocity): Last known joint_states message time stamp not found in known joint_states messages. Using all known messages." << std::endl;
                        }
                        else
                        {
        ...
        void ZyVelocityApproximator::pushJointMsg(jointMsg theMsg)
        {
        ...
        ZyVelocityApproximator::currentVelocityApproximationDataType ZyVelocityApproximator::createOutputArray(const std::pair< identificationData, velocityData>& velocityDataPair)
        {
        ...
        std::vector<std::string> ZyVelocityApproximator::generateObjectNameVector(char* vertexInput)
        {
        ...
        std::vector<double> ZyVelocityApproximator::generateVertexVector(char* vertexInput)
        {
            try
            {
            ...
            {
                std::cout << "(ZyVelocityApproximator::bwdInit) ERROR: Could not convert input " << vertexInput << " to integers. Can't run velocity approximation." << std::endl;
                runVelocityApproximation = false;
            }
            catch (std::out_of_range)
            {
                std::cout << "(ZyVelocityApproximator::bwdInit) ERROR: Input " << vertexInput << " contains an element that is too large. Can't run velocity approximation." << std::endl;
                runVelocityApproximation = false;
            }
        }

        double ZyVelocityApproximator::adjustTime(double time)
        {
            if (time > epochTime)
            {
